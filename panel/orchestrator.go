...
func parsePanelsFromArgs(args []string) ([]Panel, error) {
	if len(args) == 0 {
...
func (m *orchestrator) validateInputOutputConsistency() error {
	totalModels := len(m.chooseModels) + len(m.filterModels)
	if len(m.panels) != totalModels {
		err := fmt.Errorf("panel count mismatch: panels=%d models=%d (choose=%d filter=%d)",
			len(m.panels), totalModels, len(m.chooseModels), len(m.filterModels))
		return err
	}

	for i, panel := range m.panels {
		var expectedItems, actualItems int

		expectedItems = len(panel.Items)

		switch panel.Type {
		case PanelChoose:
			if panel.ModelIdx >= 0 && panel.ModelIdx < len(m.chooseModels) {
				actualItems = len(m.chooseModels[panel.ModelIdx].items)
			}
		case PanelFilter:
			if panel.ModelIdx >= 0 && panel.ModelIdx < len(m.filterModels) {
				actualItems = len(m.filterModels[panel.ModelIdx].filteringChoices)
			}
		}

		if expectedItems != actualItems {
			err := fmt.Errorf("panel %d (%s): expected %d items (%v), got %d",
				i, panel.Type, expectedItems, panel.Items, actualItems)
			return err
		}
	}
...
func (m *orchestrator) updateFilterModel(fm *filterModel, msg tea.Msg) tea.Cmd {
	...
	case tea.KeyMsg:
		...
		default:
			...
			if fm.fuzzy {
				fm.matches = fuzzy.FindNoSort(newValue, fm.filteringChoices)
			} else {
				fm.matches = exactMatches(newValue, fm.filteringChoices)
			}
			if newValue == "" {
...
func (m orchestrator) getResults() []string {
	var results []string
